...
function setupFetchInterceptIPC() {
    // Map wcId → handler teardown function (to avoid memory leaks on detach)
    const debuggerListeners = new Map();
    electron_1.ipcMain.handle('cdp-fetch-enable', async (_event, wcId, patterns) => {
        const wc = electron_1.webContents.fromId(wcId);
        ...
        const msgHandler = (_, method, params) => {
        ...
        if (prev)
            prev(); // remove old listener if re-attaching
    ...
    electron_1.ipcMain.handle('cdp-fetch-disable', async (_event, wcId) => {
        const wc = electron_1.webContents.fromId(wcId);
        if (!wc || wc.isDestroyed())
            return { ok: true };
        // Remove listener
    ...
    electron_1.ipcMain.handle('cdp-fetch-continue', async (_event, wcId, requestId) => {
        const wc = electron_1.webContents.fromId(wcId);
        if (!wc || wc.isDestroyed())
            return { ok: false, error: 'WebContents not found' };
        try {
    ...
    electron_1.ipcMain.handle('cdp-fetch-fail', async (_event, wcId, requestId, errorReason) => {
        const wc = electron_1.webContents.fromId(wcId);
        if (!wc || wc.isDestroyed())
            return { ok: false, error: 'WebContents not found' };
        try {
...
function setupWormholeIPC() {
     ...
     */
    electron_1.ipcMain.handle('cdp-get-listeners-batch', async (_event, wcId, nodeIds) => {
        const wc = electron_1.webContents.fromId(wcId);
        if (!wc || wc.isDestroyed())
            return [];
        const CLICK_EVENTS = new Set(['click', 'mousedown']);
        // Resolve all nodeIds in parallel — one C++-level call per node, but all concurrent
        const results = await Promise.all(nodeIds.map(async (backendNodeId) => {
            try {
...
function setupCdpIPC() {
    // Track attached debugger wcIds to prevent double-attach errors
    const attached = new Set();
    electron_1.ipcMain.handle('cdp-attach', async (_event, wcId) => {
        const wc = electron_1.webContents.fromId(wcId);
        if (!wc || wc.isDestroyed())
            return { ok: false, error: 'WebContents not found' };
    ...
    electron_1.ipcMain.handle('cdp-detach', async (_event, wcId) => {
        const wc = electron_1.webContents.fromId(wcId);
        if (!wc || !attached.has(wcId))
            return { ok: true };
        try {
            wc.debugger.detach();
        }
    ...
    electron_1.ipcMain.handle('cdp-send', async (_event, wcId, method, params) => {
        const wc = electron_1.webContents.fromId(wcId);
        if (!wc || wc.isDestroyed())
            return { ok: false, error: 'WebContents not found' };
